        ...
        if use_cache:
            if use_waypoint:
                if constant_waypoint is None:
                    ...
                    cache_zarr_path = (
                        dataset_path
                        + f".constant_{constant_waypoint}_waypoint.zarr.zip"
                    )
            else:
...
def _convert_robomimic_to_replay(
    ...
    if use_waypoint:
        # eg. the dataset_path is data/robomimic/datasets/lift/ph/image_abs.hdf5
        # then the waypoint_path is robomimic/datasets/lift/ph/low_dim.hdf5
        waypoint_path = dataset_path.replace("image_abs", "low_dim") 
        waypoint_path = waypoint_path.split("/", 1)[1]
    ...
    with h5py.File(dataset_path) as file:
        ...
        for key in tqdm(lowdim_keys + ["action"], desc="Loading lowdim data"):
            ...
            for i in range(len(demos)):
                demo = demos[f"demo_{i}"]
                if data_key == "actions" and use_waypoint:
