...
from robomimic.utils.log_utils import PrintLogger, DataLogger, WandbLogger
...
def train(config, device):
    ...
    if config.env is not None:
        if config.env.interpolator:
            # add linear interpolators for pos and ori
            env_meta["env_kwargs"]["controller_configs"]["interpolation"] = "linear"
            print("Using linear interpolation for pos and ori")
        if config.env.absolute_action:
            # absolute control
            env_meta["env_kwargs"]["controller_configs"]["control_delta"] = False
            print("Using absolute actions")
        if config.env.multiplier is not None:
            env_meta["env_kwargs"]["controller_configs"][
                "multiplier"
            ] = config.env.multiplier
            print(f"Using multiplier {config.env.multiplier}")
    ...
    if config.experiment.logging.log_wandb:
        data_logger = WandbLogger(config=config)
    else:
    ...
    acs_normalization_stats = None
    if config.train.hdf5_normalize_acs:
        acs_normalization_stats = trainset.get_acs_normalization_stats()
    ...
    for epoch in range(1, config.train.num_epochs + 1):  # epoch numbers start at 1
        ...
        if (
            ...
            rollout_model = RolloutPolicy(
                ...
                acs_normalization_stats=acs_normalization_stats,
            )

            num_episodes = config.experiment.rollout.n
            initial_state = (
                trainset.initial_state
                if config.experiment.rollout.single_traj
                else None
            )
            all_rollout_logs, video_paths, pred_states = TrainUtils.rollout_with_stats(
                ...
                n_videos=config.experiment.rollout.n_videos,
            )
            ...
            if config.experiment.rollout.single_traj:
                ...
                ax.set_ylabel("y")
                ax.set_zlabel("z")
                ax.set_title(f"Task: Lift, Data idx: 0")

                plot_3d_trajectory(ax, trainset.gt_pos, label="gt")
                plot_3d_trajectory(ax, trainset.abs_pos, label="abs_pos")
                plot_3d_trajectory(ax, pred_states, label="pred")

                # save plot
                plt.savefig(os.path.join(video_dir, f"epoch_{epoch}_plot.png"))
                plt.close(fig)
...
def main(args):
    ...
    if args.debug:
        ...
        config.train.output_dir = "debug/tmp_trained_models"
