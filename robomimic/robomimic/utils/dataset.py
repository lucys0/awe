...
from utils.utils import relabel_waypoints, accumulate_delta_actions
...
class SequenceDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        Args:
            ...
            hdf5_normalize_acs (bool): if True, normalize actions by computing the mean action and std
        ...
        self.hdf5_normalize_acs = hdf5_normalize_acs
        ...
        self.acs_normalization_stats = None
        ...
        if self.hdf5_cache_mode in ["all", "low_dim"]:
            ...
            # normalize after selecting appropriate actions
            if self.hdf5_normalize_acs:
                self.acs_normalization_stats = self.normalize_acs()
    ...
    def load_demo_info(self, filter_by_attribute=None, demos=None):
        ...
        # sort demo keys
        inds = (
            np.argsort([int(elem[5:]) for elem in self.demos])
            if not self.single_traj
            else [0]
        )  # debugging: only load a single demo
    ...
    def load_dataset_in_memory(
        ...
        for ep in LogUtils.custom_tqdm(demo_list):
            ...
            if self.abs_actions:
                all_data[ep]["obs"] = {
                    k: hdf5_file["data/{}/abs_obs/{}".format(ep, k)][()][:-1].astype(
                        "float32"
                    )
                    for k in obs_keys
                }
            ...
            if self.use_waypoints:
                if self.constant_interval is not None:
                    traj_len = all_data[ep]["attrs"]["num_samples"]
                    waypoints = np.arange(1, traj_len, self.constant_interval)
            ...
            for k in dataset_keys:
                if k in hdf5_file["data/{}".format(ep)]:
                    ...
                    if k == "actions":
                        if self.abs_actions:
                            # replace delta actions with absolute actions
                            abs_actions = hdf5_file["data/{}/abs_actions".format(ep)][
                                ()
                            ].astype("float32")
                            # shift actions by one step
                            all_data[ep][k] = np.concatenate(
                                [abs_actions[1:], abs_actions[-1:]], axis=0
                            )
                            if self.use_waypoints:
                                all_data[ep][k] = relabel_waypoints(
                                    all_data[ep][k], waypoints
                                )
                        else:  # delta actions
                            all_data[ep][k] = hdf5_file["data/{}/actions".format(ep)][
                                ()
                            ].astype("float32")
                            if self.use_waypoints:
                                # need to accumulate
                                all_data[ep][k] = accumulate_delta_actions(
                                    all_data[ep][k], waypoints
                                )
                    else:  # get other dataset keys
                        all_data[ep][k] = hdf5_file["data/{}/{}".format(ep, k)][
                            ()
                        ].astype("float32")
                else:
            ...
            # debugging: load intial state
            if self.single_traj:
                self.initial_state = dict(
                    states=hdf5_file["data/{}/states".format(ep)][()][0]
                )
                self.initial_state["model"] = hdf5_file["data/{}".format(ep)].attrs[
                    "model_file"
                ]
                self.abs_pos = hdf5_file[
                    "data/{}/abs_obs/{}".format(ep, "robot0_eef_pos")
                ][()].astype("float32")
                self.gt_pos = hdf5_file["data/{}/obs/{}".format(ep, "robot0_eef_pos")][
                    ()
                ].astype("float32")

        # compute the average number of waypoints
        if len(num_waypoints) > 0:
            print("Average number of waypoints: {}".format(np.mean(num_waypoints)))
    ...
    def normalize_acs(self):
        print("SequenceDataset: normalizing actions...")
        all_action_data = []
        for ep in self.hdf5_cache:
            all_action_data.append(self.hdf5_cache[ep]["actions"])
        all_action_data = np.concatenate(all_action_data, axis=0)

        data_mean = all_action_data.mean(axis=0, keepdims=True)
        data_std = all_action_data.std(axis=0, keepdims=True)
        data_std = np.clip(data_std, 1e-2, 10)

        stats = {"mean": data_mean.squeeze(), "std": data_std.squeeze()}
        return stats

    def get_acs_normalization_stats(self):
        """
        Returns dictionary of mean and std for each action key if using
        action normalization, otherwise None.

        Returns:
            acs_normalization_stats (dict): a dictionary for action
                normalization. This maps action keys to dicts
                with a "mean" and "std" of shape (1, ...) where ... is the default
                shape for the action.
        """
        assert self.hdf5_normalize_acs, "not using action normalization!"
        return deepcopy(self.acs_normalization_stats)
    ...
    def get_item(self, index):
        ...
        if self.hdf5_normalize_acs:
            meta["actions"] = ObsUtils.normalize_acs(
                meta["actions"], acs_normalization_stats=self.acs_normalization_stats
            )
