...
def policy_from_checkpoint(device=None, ckpt_path=None, ckpt_dict=None, verbose=False):
    ...
    acs_normalization_stats = ckpt_dict.get("acs_normalization_stats", None)
    if acs_normalization_stats is not None:
        assert config.train.hdf5_normalize_actions
        for k in acs_normalization_stats:
            acs_normalization_stats[k] = np.array(acs_normalization_stats[k])
