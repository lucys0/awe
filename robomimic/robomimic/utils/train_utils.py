...
def dataset_factory(config, obs_keys, filter_by_attribute=None, dataset_path=None):
    ...
    ds_kwargs = dict(
        ...
        hdf5_normalize_acs=config.train.hdf5_normalize_acs,
        filter_by_attribute=filter_by_attribute,
        use_waypoints=config.env.use_waypoints,
        abs_actions=config.env.absolute_action,
        single_traj=config.experiment.rollout.single_traj,
        constant_interval=config.experiment.rollout.constant_interval,
    )
...
def run_rollout(
    ...
    ob_dict = env.reset() if initial_state is None else env.reset_to(initial_state)
    goal_dict = None
    if use_goals:
    ...
    pred_states = [ob_dict["robot0_eef_pos"]]
        ...
        for step_i in range(horizon):
            ...
            if video_writer is not None:
                if video_count % video_skip == 0:
                    ...
                    # put step and action information on the frame
                    video_img = put_text(
                        video_img,
                        text=f"{step_i}: {ac}",
                        font_size=0.3,
                ...
                pred_states.append(env.get_observation()["robot0_eef_pos"])
    ...
    results["pred_states"] = pred_states
...
def rollout_with_stats(
    ...
    for env_name, env in envs.items():
        ...
        for ep_i in iterator:
            video_writer = env_video_writer if ep_i < n_videos else None
            ...
            if ep_i < n_videos:
                pred_states = rollout_info.pop("pred_states")
    ...
    return all_rollout_logs, video_paths, pred_states
...
def save_model(
    ...
    if acs_normalization_stats is not None:
        assert config.train.hdf5_normalize_acs
        acs_normalization_stats = deepcopy(acs_normalization_stats)
        params["acs_normalization_stats"] = TensorUtils.to_list(acs_normalization_stats)
