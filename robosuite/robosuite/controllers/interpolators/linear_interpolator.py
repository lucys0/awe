...
class LinearInterpolator(Interpolator):
    ...
    def __init__(
        ...
        self.total_steps = np.ceil(
            ramp_ratio * controller_freq * multiplier / policy_freq
        )  # Total num steps per interpolator action (5 by default)
        self.use_delta_goal = (
    ...
    def get_interpolated_goal(self):
        ...
        if self.ori_interpolate is not None:
            ...
            if self.ori_interpolate == "euler":
                ...
                # if x or self.goal is a euler, first convert to mat
                if len(x.shape) == 1:
                    x = T.euler2mat(x)
                x = T.mat2quat(x)
                if len(goal.shape) == 1:
                    goal = T.euler2mat(self.goal)
                goal = T.mat2quat(goal)
            ...
            if self.ori_interpolate == "euler":
                # Map back to euler
                # x_current = T.mat2euler(T.quat2mat(x_current)) # this conversion produces error
                x_current = T.quat2mat(x_current)
        else:
            # This is a normal interpolation
            # dx = (self.goal - x) / (self.total_steps - self.step)
            dx = (self.goal - x) * (self.step + 1) / self.total_steps
            x_current = x + dx
