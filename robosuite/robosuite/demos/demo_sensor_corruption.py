...
if __name__ == "__main__":
    ...
    if args.device == "keyboard":
        ...
        on_press = lambda key: device.on_press(
            None, ord(chr(key).capitalize()), None, None, None
        )
        on_release = lambda key: device.on_release(
            None, ord(chr(key).capitalize()), None, None, None
        )
    ...
    while True:
        ...
        while True:
            ...
            im = np.flip(obs[args.camera + "_image"].transpose((1, 0, 2)), 1).astype(
                np.int
            )
