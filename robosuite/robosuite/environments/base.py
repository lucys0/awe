...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def __init__(
        ...
        self._model_postprocessor = (
            None  # Function to post-process model after load_model() call
        )
        self.model = None
        self.cur_time = None
    ...
    def step(self, action, skip=None, record=False):
        """
        ...
        # Initialize lists for storing intermediate states and observations
        state_list = []
        obs_list = []

        # Loop through the simulation at the model timestep rate until we're ready to take the next policy step
        # (as defined by the control frequency specified at the environment level)
        sim_steps = int(self.control_timestep * self.multiplier / self.model_timestep)
        if skip is not None and self.multiplier is not None:
            self.robots[0].gripper.speed_multiplier = skip / self.multiplier
        ...
        for i in range(sim_steps):
            ...
            state = self._get_observations(force_update=True)

            # Store the intermediate state and observation
            if record:
                state_list.append(
                    dict(
                        robot0_eef_pos=state["robot0_eef_pos"],
                        robot0_eef_quat=state["robot0_eef_quat"],
                        robot0_joint_pos=state["robot0_joint_pos"],
                    )
                )
                obs_list.append(
                    self.sim.render(height=512, width=512, camera_name="agentview")[
                        ::-1
                    ]
                )
        ...
        # Extend info with intermediate states and observations
        info["intermediate_states"] = state_list
        info["intermediate_obs"] = obs_list
