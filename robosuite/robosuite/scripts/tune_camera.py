...
def modify_xml_for_camera_movement(xml, camera_name):
    ...
    for camera in cameras:
        if camera.get("name") == camera_name:
            ...
            break
    assert (
        camera_elem is not None
    ), "No valid camera name found, options are: {}".format(
...
def move_camera(env, direction, scale, cam_body_id):
    ...
    camera_rot = T.quat2mat(
        T.convert_quat(env.sim.model.body_quat[cam_body_id], to="xyzw")
    )
...
def rotate_camera(env, direction, angle, cam_body_id):
    ...
    # set new rotation
    env.sim.model.body_quat[cam_body_id] = T.convert_quat(
        T.mat2quat(camera_rot), to="wxyz"
    )
    env.sim.forward()
